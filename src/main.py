...
# DataFrame to aggregate text and dump into CSV
df = pd.DataFrame(columns=['Name', 'Constituency', 'Date of Birth', 'Education'])

# Get all scans in directory (planning on separate folders for each term)
img_files = listdir(DIR_NAME)

for img_file in sorted(img_files):
  # Grab scan and convert to HSV
  img = cv.imread('./img/' + img_file)
  ...
  for box in text_boxes:
    ...
    # TODO: figure out how to display image while correcting
    cv.imshow('crop', cv.resize(crop, None, fx=0.5, fy=0.5))
...
df.to_csv(f'temp/{img[2:].replace('/', '-')}.csv')      
